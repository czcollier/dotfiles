...
if &background == "dark"
  hi def otlText guibg=#000000 guifg=#999999
...
if g:otl_bold_headers
  hi def otlTab0 ctermfg=1 cterm=bold  guifg=red term=reverse
  hi def otlTab1 ctermfg=4 cterm=bold  guifg=lavender term=reverse
  hi def otlTab2 ctermfg=2 cterm=bold  guifg=darkgreen term=reverse
  hi def otlTab3 ctermfg=3 cterm=bold  guifg=brown term=reverse
  hi def otlTab4 ctermfg=5 cterm=bold  guifg=darkmagenta term=reverse
  hi def otlTab5 ctermfg=6 cterm=bold  guifg=darkcyan term=reverse
  hi def otlTab6 ctermfg=1 cterm=bold  guifg=red term=reverse
  hi def otlTab7 ctermfg=4 cterm=bold  guifg=lavender term=reverse
  hi def otlTab8 ctermfg=2 cterm=bold  guifg=darkgreen term=reverse
  hi def otlTab9 ctermfg=3 cterm=bold  guifg=brown term=reverse
else
  hi def otlTab0 ctermfg=1 cterm=NONE guifg=#FFCDCD
  hi def otlTab1 ctermfg=4 cterm=NONE guifg=lavender
  hi def otlTab2 ctermfg=2 cterm=NONE guifg=#DDFFEE
  hi def otlTab3 ctermfg=3 cterm=NONE guifg=#FFDDDD
