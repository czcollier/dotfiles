...
sign define breakpoint linehl=DebugBreak texthl=DebugBreak text=*
...
function! VDBBreakSetFull()
    let classname = substitute(substitute(bufname("%"), ".java", "", ""), "\/", ".", "g")
    call VDBCommand("stop at " . classname . ":" . line("."))
    call VDBBreakSet(s:breakpoint_no, bufname("%"), line("."))
    let s:breakpoint_no = s:breakpoint_no + 1
endfunction

function! VDBBreakDelFull()
    let classname = substitute(substitute(bufname("%"), ".java", "", ""), "\/", ".", "g")
    call VDBCommand("clear " . classname . ":" . line("."))
...
function! VDBKeyMap()
    ...
    if match(s:type, "jdb") != -1
        ...
        nmap <unique> <F9>          :execute "Vdb stop at " . substitute(substitute(bufname("%"), ".java", "", ""), "\/", ".", "g") . ":" . line(".")<CR>
        nmap <unique> <F6>          :execute VDBBreakSetFull()<CR>
        nmap <unique> <F11>         :execute VDBBreakDelFull()<CR>
        nmap <unique> <C-p>p        :Vdb print <C-R><C-W><CR>
        nmap <unique> <C-p>d        :Vdb dump <C-R><C-W><CR>
